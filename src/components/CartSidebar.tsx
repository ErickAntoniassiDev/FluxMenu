        ...
        <div className="fixed inset-0 z-50 overflow-hidden" id="cart-sidebar-wrapper">
          ...
          <div className="absolute inset-y-0 right-0 max-w-full flex pl-10 pointer-events-none">
            <motion.div
              ...
              className="w-screen max-w-md pointer-events-auto flex flex-col bg-white shadow-2xl border-l border-slate-100"
            >
              {/* Header */}
              <div className="p-6 border-b border-slate-100/80 flex items-center justify-between shrink-0">
                <div className="flex items-center gap-2">
                  <span className="p-1 rounded-lg bg-rose-50 text-rose-600 block">
                ...
                <button
                  onClick={onClose}
                  className="w-8 h-8 rounded-lg hover:bg-slate-50 border border-slate-100 text-slate-400 hover:text-slate-700 flex items-center justify-center transition-colors cursor-pointer"
                  id="close-cart-btn"
                >
                  <X className="w-4 h-4" />
                </button>
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-4">
                {cart.length === 0 ? (
                  <div className="h-full flex flex-col items-center justify-center text-center p-6 select-none">
                    <div className="w-16 h-16 rounded-full bg-slate-50 flex items-center justify-center border border-slate-100 text-slate-400 mb-4 animate-pulse">
                      🛒
                    </div>
                    <span className="font-display font-extrabold text-xs text-slate-800 uppercase tracking-wider block">Carrinho Vazio</span>
                    <p className="text-[10px] text-slate-500 max-w-[220px] leading-relaxed mt-1">
                      Explore nosso cardápio e adicione seus pratos e bebidas favoritas para iniciar um pedido.
                    </p>
                  </div>
                ) : (
                  <div className="space-y-3">
                    <div className="p-3 bg-indigo-50 border border-indigo-100/50 rounded-xl flex items-start gap-2 text-indigo-950">
                      ...
                      {cart.map((item, index) => (
                        <div key={`${item.product.id}-${index}`} className="py-4 flex gap-3 first:pt-0 last:pb-0" id={`cart-item-${item.product.id}`}>
                          ...
                          <div className="flex-1 min-w-0">
                            ...
                            {item.observation && (
                              <span className={`text-[9px] block font-medium p-1 mt-1.5 rounded-sm truncate ${
                                item.observation.includes('[ATENÇÃO ALERGIA')
                                  ? 'bg-rose-50 text-rose-700 font-black border border-rose-100/40'
                          ...
                          <div className="flex flex-col items-end gap-1.5 justify-between shrink-0 select-none">
                            <button
                              ...
                              className="text-slate-300 hover:text-rose-500 p-1 rounded-sm hover:bg-slate-50 transition-colors"
                              title="Remover Item"
                            ...
                            <div className="flex items-center gap-2 border border-slate-200 rounded-lg p-0.5 bg-slate-50 shrink-0">
                              <button
                                ...
                                className="w-5 h-5 rounded-md bg-white border border-slate-200 text-slate-700 font-mono text-[10px] font-black flex items-center justify-center hover:bg-slate-50 transition"
                              >
                                -
                              </button>
                ...
                <div className="p-6 bg-slate-50 border-t border-slate-100 shrink-0">
                  <div className="space-y-2 mb-5">
                    ...
                    <div className="flex justify-between text-xs font-medium text-slate-500">
                      ...
                      <span className="font-mono text-emerald-500">Grátis (SaaS)</span>
                    </div>
                  ...
                  <button
                    ...
                    className="w-full h-12 bg-slate-900 hover:bg-slate-800 disabled:bg-slate-400 text-white rounded-xl text-xs font-black tracking-wider flex items-center justify-center gap-2 shadow-md transition cursor-pointer"
                  ...
                  <p className="text-[9px] text-center text-slate-400 font-semibold uppercase tracking-wider mt-3">

...
export const OnboardingScreen: React.FC = () => {
  ...
  const [step, setStep] = useState(() => hasActiveRestaurant ? 5 : 0);
  ...
  return (
    <div className="h-full w-full bg-slate-50 overflow-y-auto p-4 md:p-6">
      <div className="mx-auto max-w-5xl bg-white border border-slate-200 rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="grid grid-cols-3 md:grid-cols-6 gap-1 p-3 bg-slate-50 border-b border-slate-100">{steps.map((label, index) => { const locked = (hasActiveRestaurant && index < 5) || (!hasActiveRestaurant && index === 5); return <button key={label} type="button" disabled={locked} onClick={() => { if (!locked) setStep(index); }} className={`h-8 rounded-lg text-[9px] font-black uppercase disabled:opacity-50 ${step === index ? 'bg-slate-950 text-white' : index < step ? 'bg-emerald-50 text-emerald-700' : 'bg-white text-slate-400'}`}>{label}</button>; })}</div>
        <div className="p-5 md:p-6 space-y-5">
          ...
          {step === 3 && <div className="space-y-4"><div className="rounded-xl border border-slate-200 bg-slate-50 p-3 text-xs font-semibold text-slate-600">Preencha a segunda-feira e use os botões abaixo para repetir esse horário. Depois ajuste sábado/domingo se forem diferentes.</div><div className="flex flex-wrap gap-2"><button onClick={() => copyToAll()} className="h-9 px-3 rounded-lg bg-slate-950 text-white text-[10px] font-black uppercase inline-flex items-center justify-center gap-1 leading-none"><Copy className="w-3.5 h-3.5" />Usar segunda em todos</button><button onClick={copyWeekdays} className="h-9 px-3 rounded-lg border text-[10px] font-black uppercase inline-flex items-center justify-center gap-1 leading-none">Usar segunda nos dias úteis</button></div><div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-4 gap-3">{WEEK_DAYS.map(([day, label]) => { const value = openingHours[day] ?? EMPTY_DAY; return <div key={day} className="p-3 rounded-xl border space-y-2"><div className="flex justify-between"><span className="text-[10px] font-black uppercase text-slate-500">{label}</span><label className="text-[10px] font-bold text-slate-500"><input type="checkbox" checked={value.closed} onChange={e => updateOpeningDay(day, { closed: e.target.checked })} /> Fechado</label></div><div className="grid grid-cols-2 gap-2"><input disabled={value.closed} value={value.open} onChange={e => updateOpeningDay(day, { open: e.target.value, closed: false })} className="p-2.5 rounded-lg border text-xs" placeholder="11:00" /><input disabled={value.closed} value={value.close} onChange={e => updateOpeningDay(day, { close: e.target.value, closed: false })} className="p-2.5 rounded-lg border text-xs" placeholder="23:00" /></div></div>; })}</div></div>}
